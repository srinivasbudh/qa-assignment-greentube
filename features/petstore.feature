Feature: Swagger Petstore pet API
  ...
  @api @smoke @regression @crud @PET-001
  ...
  @api @regression @negative @PET-003
  ...
  @api @regression @negative @PET-004
  Scenario: Invalid pet request body is rejected
    When I create a pet with an invalid request body
    Then the public sandbox should return its known server error

  @api @regression @negative @PET-005
  ...
  @api @regression @negative @PET-006
